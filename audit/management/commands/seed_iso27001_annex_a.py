...
class Command(BaseCommand):
    help = "Clean and seed ISO/IEC 27001:2022 Annex A controls (idempotent)."
    ...
    def handle(self, *args, **options):
        with transaction.atomic():
            ...
            legacy_standard = Standard.objects.filter(name="ISO 27001:2022")
            if legacy_standard.exists():
                legacy_ids = list(legacy_standard.values_list("id", flat=True))
                deleted_controls = StandardControl.objects.filter(standard_id__in=legacy_ids).count()
                legacy_standard.delete()
                deleted_standard = 1
            ...
            allowed_ids = {c["control_id"] for c in controls}

            duplicates = (
                StandardControl.objects.filter(standard=standard)
                .values("control_id")
                .annotate(total=Count("id"))
                .filter(total__gt=1)
            )
            for row in duplicates:
                dupes = StandardControl.objects.filter(
                    standard=standard,
            ...
            extra_controls = StandardControl.objects.filter(standard=standard).exclude(control_id__in=allowed_ids)
            removed_extras = extra_controls.count()
            if removed_extras:
                extra_controls.delete()
            ...
            for control in controls:
                ...
                if created:
                    ...
                    StandardControl.objects.filter(
                        standard=standard,
                        control_id=control["control_id"],
                    ).update(
                        title=control["title"],
            ...
            final_count = StandardControl.objects.filter(standard=standard).count()

        if deleted_standard:
            self.stdout.write("Standard deleted: ISO 27001:2022")
        else:
            self.stdout.write("Standard deleted: ISO 27001:2022 (not found)")

        self.stdout.write(f"Controls removed (legacy standard): {deleted_controls}")
        self.stdout.write(f"Controls removed (extras): {removed_extras}")
        self.stdout.write(f"Controls created: {created_count}")
        self.stdout.write(f"Controls skipped: {skipped_count}")
        self.stdout.write(f"Final control count: {final_count}")
